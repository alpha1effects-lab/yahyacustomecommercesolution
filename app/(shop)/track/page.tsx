  ...
  return (
    <div className="min-h-[70vh] max-w-[960px] mx-auto px-6 py-20">
      <div className="mb-10">
        ...
        <p className="text-black dark:text-gray-400 mt-3 max-w-xl">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-[1.2fr_1.2fr_auto] gap-4 items-end">
        <div>
          <label className="text-xs font-bold uppercase tracking-widest text-black dark:text-gray-400">Order Number</label>
          <input
            type="text"
            value={orderNumber}
        ...
        <div>
          <label className="text-xs font-bold uppercase tracking-widest text-black dark:text-gray-400">Email</label>
          <input
            type="email"
            value={email}
        ...
        <div className="mt-10 border border-gray-200 dark:border-gray-800 p-6 bg-white dark:bg-neutral-900">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-xs uppercase tracking-widest text-black dark:text-gray-400 mt-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            <div>
              <p className="text-xs uppercase tracking-widest text-black dark:text-gray-400">Tracking Number</p>
              <p className="text-black dark:text-white mt-1">{result.trackingNumber || 'Not assigned yet'}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-widest text-black dark:text-gray-400">Estimated Delivery</p>
          ...
          <div className="mt-8">
            <p className="text-xs uppercase tracking-widest text-black dark:text-gray-400 mb-3">Items</p>
            <div className="space-y-3">
              {result.items.map((item, index) => (
                <div key={`${item.name}-${index}`} className="flex items-center gap-3 text-sm border-b border-gray-100 dark:border-gray-800 pb-3">
                  ...
                  <span className="text-black dark:text-gray-400">{item.quantity} x Rs. {item.price.toLocaleString()}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                >
                  <div className="flex items-center gap-4 flex-wrap">
                    <div>
                      ...
                      <span className="text-xs text-black dark:text-gray-400">{new Date(order.createdAt).toLocaleDateString()}</span>
                    </div>
                  ...
                  <div className="px-5 pb-5 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-neutral-800/50">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 mb-4">
                      ...
                      <div className="md:col-span-2">
                        <p className="text-xs font-bold uppercase tracking-widest text-black dark:text-gray-400 mb-1">Address</p>
                        <p className="text-sm text-black dark:text-white">{order.address}, {order.city}{order.postalCode ? `, ${order.postalCode}` : ''}</p>
                      </div>
                    </div>
                    <div>
                      <p className="text-xs font-bold uppercase tracking-widest text-black dark:text-gray-400 mb-2">Items</p>
                      <div className="space-y-2">
                        {order.items.map((item, idx) => (
                          <div key={idx} className="flex items-center gap-3 text-sm">
                            ...
                            <span className="flex-grow text-black dark:text-white">{item.name} <span className="text-black dark:text-gray-400">x{item.quantity}</span></span>
